...
{
    public class LineSpreadFireObject : MonoBehaviour
    {
        ...
        void Update()
        {
            if (firing == false && LineSpreadManager.Instance.WaitTime == false)
            {
        ...
        IEnumerator FireBullet()
        {
            ...
            LineSpreadManager.Instance.WaitTime = true; // bullet에서 bullet을 발사하는 일이 끝날 때까지 대기.
            ...
            while (LineSpreadManager.Instance.OnCollision == false) // bullet이 충돌할 때까지 무한반복
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    LineSpreadBullet bullet = bulletObject.GetComponent<LineSpreadBullet>();
