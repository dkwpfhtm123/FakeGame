...
public class RainParentBullet : MonoBehaviour
{
    ...
    void Update()
    {
        if (RainManager.Instance.StopTime == false)
        {
            MoveBullet();
            if (firing == false)
            {
    ...
    public void SetVariable(Vector2 direction, float objectSpeed, FireType type)
    {
        Direction = direction;
    ...
    private IEnumerator CreateBullet() // 탄 생성 및 발사
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            RainChildBullet setBullet = bullet.GetComponent<RainChildBullet>();
            ...
            if (Up == true)
            {
                setBullet.SetVariable(GlobalClass.RotateDirection(Direction, 90.0f), 1.0f);
            }
            else
            {
                setBullet.SetVariable(GlobalClass.RotateDirection(Direction, -90.0f), 1.0f);
