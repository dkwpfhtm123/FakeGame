...
public class test_fireobject_1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (firing == false)
        {
            if (test_manager_1.Instance.WaitTime == false)
            {
    ...
    IEnumerator FireBullet()
    {
        ...
        test_manager_1.Instance.WaitTime = true; // bullet에서 bullet을 발사하는 일이 끝날 때까지 대기.
        ...
        while (test_manager_1.Instance.OnCollision == false) // bullet이 충돌할 때까지 무한반복
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                setBullet.SetValue(targetVector, 3.0f, anglePlus, false);
