...
namespace LineSpread
{
    public class Bullet : MonoBehaviour
    {
        ...
        void Update()
        {
            if (Manager.Instance.OnCollision == false) // 충돌판정이 있기 전까지 움직임.
            {
                MoveBullet();
            }
            else // 충돌판정이 왔을때
        ...
        private IEnumerator CreateBullet()
        {
            ...
            Manager.Instance.OnCollision = false; // 충돌판정을 끝내서 object에서 만들어진 bullet을 움직이게 함.
        ...
        void OnCollisionExit2D(Collision2D coll)
        {
            if (gameObject.GetComponent<NoWallDestroy>() != null) // bulletA , bulletB 구분.
            {
                ...
                {
                    Manager.Instance.OnCollision = true; // 충돌판정을 보냄.
                    Destroy(gameObject);
                }
            }
        }
